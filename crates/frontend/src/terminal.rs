...
use webshell_shared::{
    ShellOutput, TerminalInput, TerminalOpenRequest, TerminalResize, WsMessage,
};
...
pub fn Terminal(session_id: String) -> impl IntoView {
    ...
    create_effect(move |_| {
        ...
        if let Some(container) = container_ref.get() {
            ...
            let on_message = move |msg: WsMessage| {
                match msg {
                    ...
                    WsMessage::ShellExit(exit) => {
                        if exit.id == session_id_for_ws {
                            let code = exit.code.map(|c| c.to_string()).unwrap_or_else(|| "unknown".to_string());
            ...
            match WsClient::new(on_message) {
                Ok(client) => {
                    ...
                    let open_closure = Closure::wrap(Box::new(move || {
                        let _ = client_for_open.borrow().send(&WsMessage::TerminalOpen(TerminalOpenRequest {
                    ...
                    let on_data = Closure::wrap(Box::new(move |data: String| {
                        let _ = client_for_input.borrow().send(&WsMessage::TerminalInput(TerminalInput {
                    ...
                    let resize_callback = Closure::wrap(Box::new(move |_entries: js_sys::Array, _observer: web_sys::ResizeObserver| {
                        ...
                        let _ = client_for_resize.borrow().send(&WsMessage::TerminalResize(TerminalResize {
                    ...
                    let resize_observer = web_sys::ResizeObserver::new(resize_callback.as_ref().unchecked_ref()).unwrap();
                ...
                Err(e) => {
                    ...
                    xterm_rc.borrow().write("\x1b[31m[Failed to connect to server]\x1b[0m\r\n");
                }
            }
        }
    });
